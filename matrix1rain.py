...
def main():
    ...
    try:
        ...
        drops = [random.randint(-term_h, 0) if c < term_w // 2 else term_h + random.randint(20, 60) for c in range(term_w)]
        speeds = [1 for _ in range(term_w)]
        ...
        while True:
            ...
            appear = min(1.0,frame * 0.005)
            pulse = appear * (0.6 + 0.4 *math.sin(frame * 0.005))
            ...
            for c in range(min(term_w, len(drops))):
                ...
                drops[c] += 1
                if drops[c] > term_h + random.randint(1, 3):
